...
ok()   { print -r -- "  ${C_OK}✓${C_OFF} $1"; (( passed++ )); return 0 }
bad()  { print -r -- "  ${C_BAD}✗${C_OFF} $1"; note "$2"; (( failed++ )); return 0 }
warn() { print -r -- "  ${C_WARN}!${C_OFF} $1"; note "$2"; (( warned++ )); return 0 }
skip() { print -r -- "  ${C_DIM}–${C_OFF} $1"; (( skipped++ )); return 0 }
section() { print -r -- $'\n'"${C_DIM}── $1${C_OFF}" }
...
path_index() {
  local needle=$1; local -i i=1
  for p in $path; do
    [[ $p == $needle ]] && { print -r -- $i; return }
    (( i++ ))
  done
  print -r -- 0
}
...
check_tool() {
  ...
  [[ -z $got ]] && { bad "$tool not found in a fresh shell"; return }
  [[ $got == $~want ]] \
    && ok "$tool → $got" \
...
if (( brew_i == 0 )); then
  ...
  for vm in ${PYENV_ROOT:-$HOME/.pyenv}/shims ${RBENV_ROOT:-$HOME/.rbenv}/shims $HOME/.cargo/bin; do
    vm_i=$(fp_index $vm)
    (( vm_i == 0 )) && { skip "${vm:h:t}: not on PATH"; continue }
    (( vm_i < brew_i )) \
